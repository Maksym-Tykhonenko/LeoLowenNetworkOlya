...
import {
  SafeAreaView, View, Text, StyleSheet, TextInput, Pressable, ScrollView, Image, Alert,
  Platform, PermissionsAndroid, Linking, useWindowDimensions,
} from 'react-native';
...
const C = { primary: '#DB043F', stroke: '#E6E6E6', hint: '#A2A2A2', text: '#1E1E1E', muted: '#8D8D8D' };
const TAB_BAR_H = 100;
...
export default function EditGroupScreen({ route, navigation }: Props) {
  ...
  const { getById, updateGroup } = useGroups();
  ...
  useEffect(() => {
    ...
    setCoverUri(group.coverUri ?? undefined);
    setSelected(new Set(group.masterIds));
  }, [group]);
  ...
  const contentBottomPad = blockerH + s(24);
        ...
        <View style={[styles.mCard, { width: CARD_W }, chosen && styles.mCardActive]}>
          <View style={[styles.mPhoto, { height: CARD_PH }]}>
            {m.photoUri
              ...
              : <View style={{ flex: 1, borderRadius: 12, backgroundColor: '#F2F2F2' }} />}
          </View>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      <Stagger index={0} from="bottom" replayOnFocus>
        <View style={[styles.header, { paddingTop: 6 + topPad }]}>
          ...
          <Text style={[styles.headerTitle, { fontSize: s(22) }]}>Edit group</Text>
      ...
      >
        ...
        <Stagger index={masters.length + 8} from="bottom" replayOnFocus>
          {coverUri ? (
            ...
            <Pressable
              style={[styles.addCoverBtn, { width: s(64), height: s(36), borderRadius: s(18) }]}
              onPress={pickCover}
            >
