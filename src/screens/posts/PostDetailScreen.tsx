...
const isArticleId = (v: unknown): v is ArticleId =>
  typeof v === 'string' && ARTICLE_IDS.includes(v as ArticleId);
...
export default function PostDetailScreen({ navigation, route }: Props) {
  ...
  const params = route.params as { id?: string; postId?: string } | undefined;
  const id = params?.id ?? params?.postId;
  const post = id ? getPost(id) : undefined;

  if (!id || !post) {
    return (
      <SafeAreaView style={{ flex: 1, padding: 16, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center' }}>
        <Text style={{ fontWeight: '700', fontSize: 16, marginBottom: 8, color: C.text }}>Post not found</Text>
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      ...
      >
        {post.imageUri ? <Image source={{ uri: post.imageUri }} style={s.image} resizeMode="cover" /> : null}
